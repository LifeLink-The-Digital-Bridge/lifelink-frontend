...
const RecipientScreen: React.FC = () => {
  ...
  useEffect(() => {
    const checkExistingData = async () => {
      try {
        ...
        setHasExistingData(!!recipientData);
      } catch (error) {
  ...
  }, [manualLocationSet, params.selectedLatitude, params.selectedLongitude, formState.latitude, formState.longitude]);
  ...
  useEffect(() => {
    const ensureRecipientRole = async (): Promise<void> => {
      ...
      try {
        ...
        showAlert("Role Assignment Failed", error.message || "Unable to assign recipient role.", "error");
        router.replace("/(auth)/loginScreen");
        return;
      } finally {
  ...
  useEffect(() => {
    const initializeLocationOnce = async () => {
      if (locationInitialized || manualLocationSet || hasExistingData || (formState.latitude && formState.longitude)) {
        return;
      }
      ...
      try {
        ...
        if (!manualLocationSet && !hasExistingData && !formState.latitude && !formState.longitude) {
  ...
  }, [hasExistingData, manualLocationSet, formState.latitude, formState.longitude, locationInitialized]);
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      const payload = {
        ...
        medicalDetails: {
          hemoglobinLevel: formState.hemoglobinLevel ? Number(formState.hemoglobinLevel) : undefined,
          ...
          infectiousDiseaseDetails: formState.hasInfectiousDiseases ? formState.infectiousDiseaseDetails : undefined,
        ...
        eligibilityCriteria: {
          ...
          weightEligible: formState.weight ? parseFloat(formState.weight) >= 45 : false,
          ...
          lastReviewed: formState.lastReviewed || new Date().toISOString().slice(0, 10),
          isLivingDonor: false,
        },
        hlaProfile: formState.hlaA1 || formState.hlaA2 || formState.hlaB1 || formState.hlaB2 ? {
      ...
      if (response?.id) {
        ...
        await SecureStore.setItemAsync("recipientData", JSON.stringify(response));

        if (response.addresses && response.addresses.length > 0 && !formState.addressId) {
          formState.setAddressId(response.addresses[0].id || null);
        }

        showAlert("Registration Successful!", "Your recipient profile has been created successfully.", "success");

        setTimeout(() => {
          router.replace("/(tabs)/receive");
        }, 2000);
      } else {
        throw new Error("Registration succeeded but recipientId missing in response.");
      }
    } catch (error: any) {
  ...
  if (roleLoading || locationLoading) {
    const loadingMessage = roleLoading ? "Setting up recipient role..." : "Getting your location...";
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          <View style={styles.headerContainer}>
            ...
            <View style={styles.headerTextContainer}>
              ...
              <Text style={styles.headerSubtitle}>Complete your medical profile</Text>
            </View>
