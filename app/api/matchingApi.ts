...
const handleResponse = async (response: Response, errorMessage: string) => {
  if (!response.ok) {
    ...
    switch (response.status) {
      ...
      default:
        throw new Error(
          errorDetails || `${errorMessage} (Status: ${response.status})`,
        );
    }
  }
  return response;
...
export const donorConfirmMatch = async (matchId: string): Promise<string> => {
  ...
  const response = await fetch(
    `${BASE_URL}/matching/donor/confirm/${matchId}`,
    {
      method: "POST",
      headers,
    },
...
export const recipientConfirmMatch = async (
  matchId: string,
): Promise<string> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/recipient/confirm/${matchId}`,
    {
      method: "POST",
      headers,
    },
...
export const getMatchesByDonation = async (
  donationId: string,
): Promise<MatchResponse[]> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/donation/${donationId}/matches`,
    {
      method: "GET",
      headers,
    },
...
export const getMatchesByRequest = async (
  ...
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/request/${requestId}/matches`,
    {
      method: "GET",
      headers,
    },
...
export const donorRejectMatch = async (
  matchId: string,
...
export const recipientRejectMatch = async (
  matchId: string,
  reason: string = "Not specified",
): Promise<string> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/recipient/reject/${matchId}`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({ reason }),
...
export const donorWithdrawConfirmation = async (
  matchId: string,
  reason: string = "Not specified",
): Promise<string> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/donor/withdraw/${matchId}`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({ reason }),
...
export const recipientWithdrawConfirmation = async (
  matchId: string,
  reason: string = "Not specified",
): Promise<string> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/recipient/withdraw/${matchId}`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({ reason }),
  ...
  await handleResponse(
    response,
    "Failed to withdraw confirmation as recipient",
  );
  return await response.text();
};

export const recipientConfirmCompletion = async (
  matchId: string,
  details: CompletionConfirmationDTO,
): Promise<string> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/recipient/confirm-completion/${matchId}`,
    {
      method: "POST",
      headers,
...
export const canConfirmCompletion = async (
  matchId: string,
): Promise<Record<string, any>> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/recipient/can-confirm-completion/${matchId}`,
    {
      method: "GET",
      headers,
    },
...
export const getDonorDetailsByUserId = async (
  userId: string,
): Promise<DonorDTO | null> => {
  const headers = await getAuthHeaders();
...
export const getRecipientDetailsByUserId = async (
  userId: string,
): Promise<RecipientDTO | null> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/recipient-details/${userId}`,
    {
      method: "GET",
      headers,
    },
...
export const fetchDonationByIdWithAccess = async (
  donationId: string,
): Promise<DonationDTO> => {
  const headers = await getAuthHeaders();
  ...
  if (response.status === 403) {
    throw new Error(
      "Access denied - you are not authorized to view this donation",
    );
  }
...
export const fetchRequestByIdWithAccess = async (
  requestId: string,
...
export const getMatchConfirmationStatus = async (
  matchId: string,
): Promise<boolean> => {
  const headers = await getAuthHeaders();
...
export const getDonorSnapshotByDonation = async (
  donationId: string,
): Promise<DonorDTO> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/donations/${donationId}/donor-snapshot`,
    {
      method: "GET",
      headers,
    },
...
export const getRecipientSnapshotByRequest = async (
  requestId: string,
): Promise<RecipientDTO> => {
  const headers = await getAuthHeaders();
  const response = await fetch(
    `${BASE_URL}/matching/requests/${requestId}/recipient-snapshot`,
    {
      method: "GET",
      headers,
    },
