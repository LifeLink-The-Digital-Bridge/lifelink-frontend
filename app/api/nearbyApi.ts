...
import { NearbyDonationActivityDTO, NearbyRequestActivityDTO } from './nearbyTypes';
...
export const getNearbyDonations = async (
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to fetch nearby donations: ${response.status} ${response.statusText}`);
    }
...
export const getNearbyRequests = async (
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to fetch nearby requests: ${response.status} ${response.statusText}`);
    }
